...
class ArrowPositioningService(IArrowPositioningService):
    ...
    def _calculate_dash_arrow_location(
        ...
        if (
            pictograph_data
            and motion.turns == 0
            and self._is_type3_scenario(pictograph_data)
        ):

            # Get the other motion (shift motion)
            other_motion = self._get_other_motion(motion, pictograph_data)
            if other_motion:
                # Use Type 3 logic with DIAMOND grid (default for TKA)
                return self.dash_location_service.calculate_dash_location_for_type3(
                    motion, other_motion, grid_mode="diamond"
                )
        ...
        return self.dash_location_service.calculate_dash_location(
            ...
            grid_mode="diamond",  # Use diamond as default
        )
