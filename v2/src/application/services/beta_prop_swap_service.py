...
class BetaPropSwapService:
                    ...
                    for json_file in subfolder_path.glob("*_placements.json"):
                        try:
                            with open(json_file, 'r', encoding='utf-8') as f:
                                data = json.load(f)
                                self._special_placements[grid_mode][subfolder].update(data)
                        except Exception as e:
                            print(f"⚠️ Error loading {json_file}: {e}")
            
            print(f"✅ Loaded special placements for beta prop swap overrides")
    ...
    def should_swap_beta_props(
        ...
        """
        if not self._special_placements or not beat_data.blue_motion or not beat_data.red_motion:
            return False
        ...
        try:
            ...
            if swap_flag:
                print(f"🔄 Beta swap override found: {override_key}")
                return True
