...
class UIStateManagementService(IUIStateManagementService):
    ...
    def __init__(self):
        ...
        # Settings file path
        self._settings_file = Path("user_settings.json")
    ...
    def get_setting(self, key: str, default: Any = None) -> Any:
        """Get a setting value."""
        return self._ui_state.user_settings.get(key, self._default_settings.get(key, default))
    ...
    def set_graph_editor_height(self, height: int) -> None:
        ...
        self._ui_state.graph_editor_height = max(100, min(800, height))  # Clamp between 100-800
        self._save_state()
        ...
        if self._settings_file.exists():
            try:
                ...
                if "component_visibility" in data:
                    self._ui_state.component_visibility.update(data["component_visibility"])
