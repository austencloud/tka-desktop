...
This service provides access to the actual v1 pictograph dataset,
...
from ...domain.models.core_models import (
    BeatData, MotionData, MotionType, RotationDirection, Location, GlyphData
)
...
class PictographDatasetService:
    """
    Service for accessing the v1 pictograph dataset.
    ...
    def _load_datasets(self) -> None:
        ...
        try:
            ...
            assets_dir = Path(__file__).parent.parent.parent / "infrastructure" / "assets"
            
            diamond_path = assets_dir / "DiamondPictographDataframe.csv"
            box_path = assets_dir / "BoxPictographDataframe.csv"
    ...
    def get_start_position_pictograph(self, position_key: str, grid_mode: str = "diamond") -> Optional[BeatData]:
        ...
        try:
            ...
            dataset = self._diamond_dataset if grid_mode == "diamond" else self._box_dataset
            ...
            if matching_entries.empty:
                print(f"❌ No start position found for {position_key} in {grid_mode} dataset")
                return None
    ...
    def _dataset_entry_to_beat_data(self, entry: pd.Series) -> BeatData:
        ...
        blue_motion = MotionData(
            ...
            start_ori="in",
            end_ori="out" if blue_motion_type in [MotionType.PRO, MotionType.ANTI] else "in"
        )
        ...
        red_motion = MotionData(
            ...
            start_ori="in",
            end_ori="out" if red_motion_type in [MotionType.PRO, MotionType.ANTI] else "in"
        )
    ...
    def _parse_location(self, location_str: str) -> Location:
        ...
        location_map = {
            "n": Location.NORTH, "ne": Location.NORTHEAST, "e": Location.EAST, "se": Location.SOUTHEAST,
            "s": Location.SOUTH, "sw": Location.SOUTHWEST, "w": Location.WEST, "nw": Location.NORTHWEST
        }
