...
class OptionPickerStateService(QObject):
    ...
    def initialize(self) -> None:
        ...
        print(
            "🎯 Option picker state service initialized - showing start position picker"
        )
    ...
    def is_ready_for_options(self) -> bool:
        ...
        return (
            ...
            and self._selected_start_position is not None
        )
    ...
    def _beat_data_to_dict(self, beat_data: BeatData) -> Dict[str, Any]:
        ...
        return {
            ...
            "blue_attributes": {
                "start_loc": (
                    beat_data.blue_motion.start_loc.value
                ...
                "start_ori": (
                    beat_data.blue_motion.start_ori if beat_data.blue_motion else "in"
                ),
                "end_ori": (
                    beat_data.blue_motion.end_ori if beat_data.blue_motion else "in"
                ),
                "prop_rot_dir": (
                    beat_data.blue_motion.prop_rot_dir.value
                    if beat_data.blue_motion
                    else "no_rot"
                ),
            ...
            "red_attributes": {
                "start_loc": (
                    beat_data.red_motion.start_loc.value
                    if beat_data.red_motion
                    else "s"
                ),
                "end_loc": (
                    beat_data.red_motion.end_loc.value if beat_data.red_motion else "s"
                ),
                "start_ori": (
                    beat_data.red_motion.start_ori if beat_data.red_motion else "in"
                ),
                "end_ori": (
                    beat_data.red_motion.end_ori if beat_data.red_motion else "in"
                ),
                "prop_rot_dir": (
                    beat_data.red_motion.prop_rot_dir.value
                    if beat_data.red_motion
                    else "no_rot"
                ),
