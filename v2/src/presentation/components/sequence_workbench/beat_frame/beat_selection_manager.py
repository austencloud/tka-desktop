...
replacing V1's BeatSelectionOverlay with modern state management patterns.
"""
...
class BeatSelectionManager(QObject):
    ...
    Replaces V1's BeatSelectionOverlay with:
    ...
    def register_beat_views(self, beat_views: List[ModernBeatView]):
    ...
    def remove_from_selection(self, beat_index: int):
        ...
        if beat_index in self._selected_indices:
            ...
            if self._selected_index == beat_index:
                self._selected_index = self._selected_indices[0] if self._selected_indices else None
    ...
    def focus_selected_beat(self):
        ...
        if self._selected_index is not None and self._selected_index < len(self._beat_views):
