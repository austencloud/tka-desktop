...
def test_full_option_picker_workflow():
    ...
    try:
        ...
        from presentation.components.option_picker import ModernOptionPicker
        ...
        if beat_options:
                        ...
                        if container and hasattr(container, 'layout'):
                            ...
                            if layout:
                                ...
                                if child_count > 0:
                                    ...
                                    print(f"     {section_name}: {child_count} pictographs")
            ...
            if beat_options:
                ...
                print(f"   Blue start→end: {sample_beat.blue_motion.start_loc} → {sample_beat.blue_motion.end_loc}")
                print(f"   Red start→end: {sample_beat.red_motion.start_loc} → {sample_beat.red_motion.end_loc}")
...
def test_position_matching_performance():
    ...
    try:
        ...
        print(f"   Batch testing ({len(positions_to_test)} positions): {batch_time:.3f}s")
        print(f"   Total options found: {total_options}")
        print(f"   Average per position: {total_options/len(positions_to_test):.1f} options")
