...
from actions import UserAction, ActionType, GridPosition, ActionParameters, ExpectedOutcome
...
async def example_single_action_test():
    ...
    try:
        ...
        if result["success"]:
            ...
            print(f"   V1 Success: {result['v1_result'].success}")
...
async def example_custom_scenario():
    ...
    custom_scenario.add_action(UserAction(
    ...
    custom_scenario.add_action(UserAction(
        ...
        expected_outcome=ExpectedOutcome(
            expected_data={"start_position": "beta"}
        )
    ))
    ...
    try:
        ...
        for i, action in enumerate(custom_scenario.actions):
            print(f"  Action {i+1}/{len(custom_scenario.actions)}: {action.description}")
        ...
        for i, comparison in enumerate(result.comparison_results):
            ...
            if comparison.differences:
                ...
                for diff in comparison.differences[:3]:  # Show first 3
                    print(f"      - {diff.field_path}: {diff.v1_value} vs {diff.v2_value}")
