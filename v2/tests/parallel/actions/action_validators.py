...
PURPOSE: Validate action prerequisites and outcomes for V1/V2 testing
"""
...
class IActionValidator(ABC):
    ...
    @abstractmethod
    def validate_prerequisites(self, action: UserAction, state: Dict[str, Any]) -> ValidationResult:
    ...
    @abstractmethod
    def validate_outcome(self, action: UserAction, result: Dict[str, Any]) -> ValidationResult:
...
class BaseActionValidator(IActionValidator):
    ...
    def validate_prerequisites(self, action: UserAction, state: Dict[str, Any]) -> ValidationResult:
        ...
        if action.action_type in [ActionType.ADD_BEAT, ActionType.DELETE_BEAT, ActionType.MODIFY_MOTION]:
    ...
    def validate_outcome(self, action: UserAction, result: Dict[str, Any]) -> ValidationResult:
        ...
        if not result.get("success", False):
            validation_result.add_error(f"Action execution failed: {result.get('error', 'Unknown error')}")
...
class StartPositionValidator(BaseActionValidator):
    ...
    def validate_prerequisites(self, action: UserAction, state: Dict[str, Any]) -> ValidationResult:
        ...
        if current_beats > 0:
            result.add_warning("Selecting start position with existing beats may clear sequence")
    ...
    def validate_parameters(self, action: UserAction) -> ValidationResult:
        ...
        if action.parameters.grid_position not in [GridPosition.ALPHA, GridPosition.BETA, GridPosition.GAMMA]:
    ...
    def validate_parameters(self, action: UserAction) -> ValidationResult:
        ...
        if action.action_type == ActionType.ADJUST_TURNS:
            ...
            if turns is not None and not (-10.0 <= turns <= 10.0):
                result.add_warning(f"Turn value {turns} is outside typical range [-10.0, 10.0]")
...
class GraphEditorValidator(BaseActionValidator):
    ...
    def validate_prerequisites(self, action: UserAction, state: Dict[str, Any]) -> ValidationResult:
        ...
        if action.action_type in [ActionType.GRAPH_EDITOR_TURN_ADJUSTMENT, ActionType.GRAPH_EDITOR_MOTION_CHANGE]:
            if not state.get("graph_editor_open", False):
                result.add_error("Graph editor must be open for graph editor operations")
...
class ActionValidatorFactory:
    ...
    @classmethod
    def validate_action(cls, action: UserAction, state: Dict[str, Any]) -> ValidationResult:
