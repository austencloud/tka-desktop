...
Deploy and run TKA V1 and V2 applications side-by-side for visual parallel testing.
Opens V1 on one monitor and V2 on another monitor for real-time comparison.

LIFECYCLE: SCAFFOLDING
DELETE_AFTER: V1 deprecation complete
PURPOSE: Visual validation of V1/V2 functional equivalence
"""
...
class SideBySideTestDeployer:
    """Deploys V1 and V2 applications side-by-side for visual testing."""
        ...
        try:
            ...
            if screen_count >= 2:
                ...
                if secondary_screen:
                    ...
                    print(f"   📺 Primary Monitor: {primary_geometry.width()}x{primary_geometry.height()} at ({primary_geometry.x()}, {primary_geometry.y()})")
                    print(f"   📺 Secondary Monitor: {secondary_geometry.width()}x{secondary_geometry.height()} at ({secondary_geometry.x()}, {secondary_geometry.y()})")
                    
                    # V1 on primary monitor (left side)
            ...
            if screen_count == 1:
                ...
                print(f"   📺 Single Monitor: {primary_geometry.width()}x{primary_geometry.height()}")
    ...
    async def deploy_applications(self) -> bool:
        """Deploy both V1 and V2 applications with optimal window positioning."""
    ...
    async def _position_windows(self):
        """Position V1 and V2 windows for optimal side-by-side viewing."""
        try:
            # Give applications time to fully load
            await asyncio.sleep(3)
            
            # Position V1 window
            if self.orchestrator.v1_driver.main_window and self.v1_window_geometry:
                v1_window = self.orchestrator.v1_driver.main_window
                v1_window.setGeometry(self.v1_window_geometry)
                v1_window.setWindowTitle("TKA V1 - Parallel Testing")
                v1_window.raise_()
                print(f"   📍 V1 positioned at: {self.v1_window_geometry}")
    ...
    async def run_visual_test_scenario(self, scenario_name: str):
        ...
        try:
            ...
            print(f"⏱️  Estimated Duration: {scenario.estimated_duration_ms / 1000:.1f} seconds")
            ...
            for i, action in enumerate(scenario.actions):
                ...
                if result["success"]:
                    ...
                    print(f"   📊 Equivalence: {result['comparison_result'].equivalence_score:.2%}")
                else:
