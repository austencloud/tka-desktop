...
class TestMigrator:
    ...
    def __init__(self, tests_root: Path = None):
        ...
        self.debug_patterns = [
            r"debug", r"crash", r"bug", r"fix", r"issue", r"critical",
            r"simple", r"minimal", r"reproduce", r"test_.*_debug"
        ]
        
        self.exploration_patterns = [
            r"exploration", r"understand", r"v1.*behavior", r"parity.*test",
            r"investigate", r"analyze", r"study"
        ]
        
        self.spike_patterns = [
            r"spike", r"prototype", r"proof.*concept", r"poc", r"experiment",
            r"trial", r"test.*concept"
        ]
        
        self.specification_patterns = [
            r"contract", r"behavior", r"specification", r"requirement",
            r"interface", r"api", r"workflow", r"immutability"
        ]
        
        self.regression_patterns = [
            r"regression", r"prevent", r"issue_\d+", r"bug_\d+", r"fix_\d+"
        ]
    ...
    def analyze_test_file(self, file_path: Path) -> Dict[str, any]:
        ...
        try:
            ...
            debug_score = self._calculate_pattern_score(filename + content.lower(), self.debug_patterns)
            if debug_score > 0.3:
                analysis["suggested_category"] = "scaffolding"
                analysis["confidence"] = debug_score
                analysis["reasons"].append(f"Debug patterns detected (score: {debug_score:.2f})")
            
            # Check for exploration patterns
            exploration_score = self._calculate_pattern_score(filename + content.lower(), self.exploration_patterns)
            if exploration_score > 0.3:
                analysis["suggested_category"] = "scaffolding"
                analysis["confidence"] = max(analysis["confidence"], exploration_score)
                analysis["reasons"].append(f"Exploration patterns detected (score: {exploration_score:.2f})")
            
            # Check for spike patterns
            spike_score = self._calculate_pattern_score(filename + content.lower(), self.spike_patterns)
            if spike_score > 0.3:
                analysis["suggested_category"] = "scaffolding"
                analysis["confidence"] = max(analysis["confidence"], spike_score)
                analysis["reasons"].append(f"Spike patterns detected (score: {spike_score:.2f})")
            
            # Check for regression patterns
            regression_score = self._calculate_pattern_score(filename + content.lower(), self.regression_patterns)
            if regression_score > 0.4:
                analysis["suggested_category"] = "regression"
                analysis["confidence"] = regression_score
                analysis["reasons"].append(f"Regression patterns detected (score: {regression_score:.2f})")
            
            # Check for specification patterns
            spec_score = self._calculate_pattern_score(filename + content.lower(), self.specification_patterns)
            if spec_score > 0.3 and analysis["suggested_category"] == "specification":
                analysis["confidence"] = spec_score
                analysis["reasons"].append(f"Specification patterns detected (score: {spec_score:.2f})")
            ...
            if file_age_days < 7 and "debug" in filename:
                ...
                analysis["reasons"].append(f"Recent debug file (age: {file_age_days} days)")
    ...
    def create_migration_report(self, plan: Dict[str, List[Dict]]) -> str:
        ...
        for category, files in plan.items():
            ...
            for analysis in files:
                ...
                report += f"- **Suggested Category**: {analysis['suggested_category']}\n"
                ...
                if analysis["existing_metadata"]:
                    report += f"- **Existing Metadata**: {analysis['existing_metadata']}\n"
    ...
    def _suggest_target_subdir(self, analysis: Dict) -> str:
        ...
        if category == "scaffolding":
            if any(pattern in filename for pattern in ["debug", "crash", "bug", "fix"]):
                return "debug"
            elif any(pattern in filename for pattern in ["exploration", "understand", "v1"]):
                return "exploration"
            ...
            if any(pattern in filename for pattern in ["domain", "model", "sequence"]):
                ...
                return "application"
            elif any(pattern in filename for pattern in ["ui", "component", "view", "frame"]):
                return "presentation"
            else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Migrate existing tests to lifecycle structure")
