#!/usr/bin/env python3
"""
Debug script to investigate V2 position glyph data extraction issues.
...
def debug_position_glyph_data():
    ...
    print("🔍 DEBUGGING V2 POSITION GLYPH DATA EXTRACTION")
    print("=" * 60)
    ...
    try:
        ...
        if sample_beat.blue_motion and sample_beat.red_motion:
            ...
            blue_start_str = blue_start.value.lower() if hasattr(blue_start, 'value') else str(blue_start).lower()
            blue_end_str = blue_end.value.lower() if hasattr(blue_end, 'value') else str(blue_end).lower()
            red_start_str = red_start.value.lower() if hasattr(red_start, 'value') else str(red_start).lower()
            red_end_str = red_end.value.lower() if hasattr(red_end, 'value') else str(red_end).lower()
