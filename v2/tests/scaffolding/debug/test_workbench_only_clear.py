#!/usr/bin/env python3
"""
Workbench-Only Clear Test for TKA V2
====================================

Tests the clear sequence operation using only the workbench component,
bypassing the construct tab to isolate the crash point.
"""
...
try:
    ...
    from src.application.services.workbench_services import (
        SequenceWorkbenchService, FullScreenService, BeatDeletionService,
        DictionaryService
    )
...
def test_workbench_only_clear():
    ...
    try:
        ...
        print(f"📊 Sequence set: {current_sequence.length if current_sequence else 0} beats")
        ...
        try:
            ...
            if not timeout_occurred:
                ...
                if cleared_length == 0:
                    print("✅ CLEAR SEQUENCE FIX CONFIRMED: Operation completed successfully!")
                    return True
                else:
