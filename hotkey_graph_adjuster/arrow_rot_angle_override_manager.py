...
class ArrowRotAngleOverrideManager:
    ...
    def __init__(self, hotkey_graph_adjuster: "HotkeyGraphAdjuster") -> None:
        ...
        self.view = hotkey_graph_adjuster.view

        self.arrow_placement_manager = (
            self.view.get_current_pictograph().managers.arrow_placement_manager
        )
    ...
    def handle_arrow_rot_angle_override(self) -> None:
        ...
        letter = self.view.get_current_pictograph().state.letter
    ...
    def _apply_override_if_needed(
        ...
        turns_tuple = TurnsTupleGenerator().generate_turns_tuple(
            self.hotkey_graph_adjuster.view.get_current_pictograph()
        )
    ...
    def _apply_rotation_override(
        ...
        letter_data = data[self.view.get_current_pictograph().state.grid_mode][
            ori_key
        ].get(letter_enum.value, {})
        turn_data = letter_data.get(turns_tuple, {})
        letter_data[turns_tuple] = turn_data
        data[self.hotkey_graph_adjuster.view.get_current_pictograph().state.grid_mode][
            ori_key
    ...
    def _update_mirrored_entry_with_rotation_override(self, updated_turn_data: dict):
        mirrored_entry_manager = (
            self.hotkey_graph_adjuster.view.get_current_pictograph().managers.arrow_placement_manager.data_updater.mirrored_entry_manager
        )
