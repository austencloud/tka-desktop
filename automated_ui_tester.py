...
Fully automated testing system that programmatically interacts with V1 and V2
applications to test specific functionality issues without manual intervention.

LIFECYCLE: SCAFFOLDING
DELETE_AFTER: V1 deprecation complete
PURPOSE: Automated V1/V2 functional equivalence validation
"""
...
# Add both V1 and V2 source paths
...
class V1ApplicationController:
    """Controller for automated V1 application interaction."""
    ...
    def _discover_components(self):
        """Discover V1 application components."""
        try:
            ...
            print(f"🔍 V1 sequence_workbench: {self.sequence_workbench}")
            ...
            if self.sequence_workbench:
                ...
                print(f"🔍 V1 option_picker: {self.option_picker}")
                print(f"🔍 V1 beat_frame: {self.beat_frame}")
        
        except Exception as e:
            print(f"❌ V1 component discovery failed: {e}")
        ...
        try:
            ...
            for button in buttons:
                if position.lower() in button.text().lower():
                    print(f"🎯 V1: Clicking start position {position}")
                    QTest.mouseClick(button, Qt.MouseButton.LeftButton)
                    QApplication.processEvents()
                    time.sleep(1)  # Wait for UI update
                    return True
            
            print(f"❌ V1: Start position button '{position}' not found")
            return False
        
        except Exception as e:
            print(f"❌ V1: Failed to click start position: {e}")
            return False
    ...
    def click_option_picker_item(self, index: int = 0) -> bool:
        ...
        try:
            ...
            if index < len(buttons):
                ...
                print(f"🎯 V1: Clicking option picker item {index} ({button.text()})")
                QTest.mouseClick(button, Qt.MouseButton.LeftButton)
                QApplication.processEvents()
                time.sleep(1)  # Wait for UI update
                return True
            
            print(f"❌ V1: Option picker item {index} not found")
            return False
        
        except Exception as e:
            print(f"❌ V1: Failed to click option picker item: {e}")
            return False
    ...
    def click_clear_sequence(self) -> bool:
        ...
        try:
            ...
            if button:
                print(f"🎯 V1: Clicking clear sequence")
                QTest.mouseClick(button, Qt.MouseButton.LeftButton)
                QApplication.processEvents()
                time.sleep(1)  # Wait for UI update
                return True
            
            print(f"❌ V1: Clear sequence button not found")
            return False
        
        except Exception as e:
            print(f"❌ V1: Failed to click clear sequence: {e}")
            return False
    ...
    def get_current_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            state["option_picker_items"] = [btn.text() for btn in option_buttons[:10]]  # First 10
            
            return state
        
        except Exception as e:
            print(f"❌ V1: Failed to get current state: {e}")
            return {}
...
class V2ApplicationController:
    ...
    def get_option_picker_buttons(self) -> List[QPushButton]:
        ...
        return UIElementFinder.find_all_buttons(self.option_picker) if self.option_picker else []
...
class AutomatedUITester:
    ...
    def __init__(self):
        self.v1_controller: Optional[V1ApplicationController] = None
        self.v2_controller: Optional[V2ApplicationController] = None
        self.test_results: List[Dict[str, Any]] = []
    
    def setup_controllers(self, v1_window, v2_window):
        """Setup controllers for both applications."""
        self.v1_controller = V1ApplicationController(v1_window)
    ...
    async def test_option_picker_dynamic_updates(self) -> Dict[str, Any]:
        ...
        try:
            ...
            v1_initial = self.v1_controller.get_current_state()
            v2_initial = self.v2_controller.get_current_state()
            
            test_result["v1_data"]["initial"] = v1_initial
            test_result["v2_data"]["initial"] = v2_initial
            
            print(f"   V1 initial options: {v1_initial.get('option_picker_count', 0)}")
            ...
            v1_start_success = self.v1_controller.click_start_position("alpha1_alpha1")
            ...
            v1_after_start = self.v1_controller.get_current_state()
            v2_after_start = self.v2_controller.get_current_state()
            
            test_result["v1_data"]["after_start"] = v1_after_start
            test_result["v2_data"]["after_start"] = v2_after_start
            
            print(f"   V1 options after start: {v1_after_start.get('option_picker_count', 0)}")
            print(f"   V2 options after start: {v2_after_start.get('option_picker_count', 0)}")
            
            # Step 4: Select first beat from option picker
            print("📊 Step 4: Selecting first beat from option picker...")
            v1_beat_success = self.v1_controller.click_option_picker_item(0)
            ...
            v1_after_beat = self.v1_controller.get_current_state()
            v2_after_beat = self.v2_controller.get_current_state()
            
            test_result["v1_data"]["after_beat"] = v1_after_beat
            test_result["v2_data"]["after_beat"] = v2_after_beat
            
            print(f"   V1 options after beat: {v1_after_beat.get('option_picker_count', 0)}")
            print(f"   V2 options after beat: {v2_after_beat.get('option_picker_count', 0)}")
            ...
            v1_updated = (v1_after_beat.get('option_picker_count', 0) != 
                         v1_after_start.get('option_picker_count', 0))
            v2_updated = (v2_after_beat.get('option_picker_count', 0) != 
                         v2_after_start.get('option_picker_count', 0))
            
            test_result["comparison"] = {
                "v1_option_picker_updated": v1_updated,
                "v2_option_picker_updated": v2_updated,
                "both_updated": v1_updated and v2_updated,
                "equivalence": v1_updated == v2_updated
            }
            
            # Identify issues
            if not v2_updated and v1_updated:
                test_result["issues_found"].append(
                    "V2 option picker failed to update after beat selection (V1 updated correctly)"
                )
            
            if not v1_updated and not v2_updated:
                test_result["issues_found"].append(
                    "Neither V1 nor V2 option picker updated after beat selection"
                )
    ...
    async def test_sequence_clear_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            v1_after_clear = self.v1_controller.get_current_state()
            v2_after_clear = self.v2_controller.get_current_state()
            
            test_result["v1_data"]["after_clear"] = v1_after_clear
            ...
            v1_has_start_buttons = v1_after_clear.get('start_buttons_count', 0) > 0
            v2_has_start_buttons = v2_after_clear.get('start_buttons_count', 0) > 0
            
            test_result["comparison"] = {
                "v1_returned_to_start_selection": v1_has_start_buttons,
                "v2_returned_to_start_selection": v2_has_start_buttons,
                "both_returned_to_start": v1_has_start_buttons and v2_has_start_buttons,
                "equivalence": v1_has_start_buttons == v2_has_start_buttons
            }
            
            # Identify issues
            if not v2_has_start_buttons and v1_has_start_buttons:
                test_result["issues_found"].append(
                    "V2 failed to return to start position selection after clear (V1 returned correctly)"
                )
            
            if not v1_has_start_buttons and not v2_has_start_buttons:
                test_result["issues_found"].append(
                    "Neither V1 nor V2 returned to start position selection after clear"
                )
...
async def main():
    ...
    try:
        # Create applications
        print("🚀 Starting V1 and V2 applications...")
        
        # Import and create V1 application
        from main_window.main_window import MainWindow as V1MainWindow
        ...
        profiler = Profiler()
        v1_window = V1MainWindow(profiler, splash_screen)
        v1_window.initialize_widgets()
        v1_window.show()
        ...
        tester.setup_controllers(v1_window, v2_window)
