...
class OptionPickerReactivityTest(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # Status label
        self.status_label = QLabel("Testing Option Picker Reactivity - Enhanced Signal Flow")
    ...
    def _check_picker_state_after_clear(self):
        ...
        try:
            if (hasattr(self.construct_tab, 'layout_manager') and 
                hasattr(self.construct_tab.layout_manager, 'picker_stack')):
                
                current_index = self.construct_tab.layout_manager.picker_stack.currentIndex()
                
                if current_index == 0:
                    print("✅ SUCCESS: Picker correctly switched to start position picker (index 0)")
                    self.status_label.setText("✅ Clear test passed: Start position picker active")
                    self.status_label.setStyleSheet("font-weight: bold; color: green;")
                else:
                    print(f"❌ FAILURE: Picker still on index {current_index} (should be 0)")
                    self.status_label.setText(f"❌ Clear test failed: Picker index {current_index}")
                    self.status_label.setStyleSheet("font-weight: bold; color: red;")
            else:
    ...
    def _check_refresh_rate(self):
        ...
        if self.refresh_count > 3:
            print(f"⚠️ WARNING: Detected {self.refresh_count} refreshes in the last second (possible cascade)")
            self.status_label.setText(f"⚠️ Warning: {self.refresh_count} refreshes detected")
