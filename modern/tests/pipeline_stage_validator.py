...
class PipelineStageResult:
    ...
    modern_dimensions: Dict[str, Any]
...
class PipelineStageValidator:
    ...
    def validate_stage_1_initialization(self) -> PipelineStageResult:
        ...
        try:
            ...
            modern_dims["scene_rect"] = {
                "width": self.modern_pictograph.scene.sceneRect().width(),
                "height": self.modern_pictograph.scene.sceneRect().height(),
            }
            modern_dims["component_size"] = {
                "width": self.modern_pictograph.size().width(),
                "height": self.modern_pictograph.size().height(),
            }

            # Check for discrepancies
            if legacy_dims["scene_rect"] != modern_dims["scene_rect"]:
                discrepancies.append(
                    f"Scene rect mismatch: Legacy={legacy_dims['scene_rect']}, Modern={modern_dims['scene_rect']}"
                )

            if legacy_dims["component_size"] != modern_dims["component_size"]:
                discrepancies.append(
                    f"Component size mismatch: Legacy={legacy_dims['component_size']}, Modern={modern_dims['component_size']}"
                )
        ...
        result = PipelineStageResult(
            stage_name, legacy_dims, modern_dims, discrepancies
        )
    ...
    def validate_stage_2_content_update(self) -> PipelineStageResult:
        ...
        try:
            ...
            modern_dims["items_count"] = len(self.modern_pictograph.scene.items())
            modern_dims["scene_rect"] = {
                "width": self.modern_pictograph.scene.sceneRect().width(),
                "height": self.modern_pictograph.scene.sceneRect().height(),
            }
            ...
            # Check for discrepancies
            if (
                abs(legacy_dims["items_count"] - modern_dims["items_count"]) > 2
            ):  # Allow small difference
                discrepancies.append(
                    f"Item count significant difference: Legacy={legacy_dims['items_count']}, Modern={modern_dims['items_count']}"
                )
    ...
    def validate_stage_3_view_scaling(self) -> PipelineStageResult:
        ...
        try:
            ...
            modern_dims["transform_scale"] = {
                "x": modern_transform.m11(),
                "y": modern_transform.m22(),
            }
            modern_dims["view_rect"] = {
                "width": self.modern_pictograph.viewport().size().width(),
                "height": self.modern_pictograph.viewport().size().height(),
            }
            ...
            modern_effective_width = (
                modern_scene_rect.width() * modern_dims["transform_scale"]["x"]
            )
            modern_effective_height = (
                modern_scene_rect.height() * modern_dims["transform_scale"]["y"]
            )
            modern_dims["effective_size"] = {
                "width": modern_effective_width,
                "height": modern_effective_height,
            }

            # Check for scaling discrepancies
            scale_diff_x = abs(
                legacy_dims["transform_scale"]["x"]
                - modern_dims["transform_scale"]["x"]
            )
            scale_diff_y = abs(
                legacy_dims["transform_scale"]["y"]
                - modern_dims["transform_scale"]["y"]
            )

            if scale_diff_x > 0.01 or scale_diff_y > 0.01:  # 1% tolerance
                discrepancies.append(
                    f"Transform scale difference: Legacy={legacy_dims['transform_scale']}, Modern={modern_dims['transform_scale']}"
                )

            # Check effective size difference
            size_diff_w = abs(legacy_effective_width - modern_effective_width)
            size_diff_h = abs(legacy_effective_height - modern_effective_height)

            if size_diff_w > 10 or size_diff_h > 10:  # 10px tolerance
                discrepancies.append(
                    f"Effective size difference: Legacy={legacy_dims['effective_size']}, Modern={modern_dims['effective_size']}"
                )
                    ...
                    if hasattr(item, "childItems") and item.childItems():
                        ...
                        if len(children) > 0:
                            ...
                            if hasattr(first_child, "boundingRect"):
                                modern_tka_found = True
                                item_rect = item.boundingRect()
                                item_pos = item.pos()
                                scene_rect = item.sceneBoundingRect()

                                modern_dims["tka_bounding_rect"] = {
                                    "width": item_rect.width(),
                                    "height": item_rect.height(),
                                }
                                modern_dims["tka_position"] = {
                                    "x": item_pos.x(),
                                    "y": item_pos.y(),
                                }
                                modern_dims["tka_scene_rect"] = {
                                    "width": scene_rect.width(),
                                    "height": scene_rect.height(),
                                    "x": scene_rect.x(),
                                    "y": scene_rect.y(),
                                ...
                                modern_dims["letter_rect"] = {
                                    "width": letter_rect.width(),
                                    "height": letter_rect.height(),
                                }
                                break

            if not modern_tka_found:
                modern_dims["tka_found"] = False
                discrepancies.append("Modern TKA glyph not found")

            # Compare TKA dimensions if both found
            if legacy_dims.get("tka_bounding_rect") and modern_dims.get(
                "tka_bounding_rect"
            ):
                legacy_tka = legacy_dims["tka_bounding_rect"]
                modern_tka = modern_dims["tka_bounding_rect"]

                width_diff = abs(legacy_tka["width"] - modern_tka["width"])
                height_diff = abs(legacy_tka["height"] - modern_tka["height"])

                if width_diff > 5 or height_diff > 5:  # 5px tolerance
                    discrepancies.append(
                        f"TKA size difference: Legacy={legacy_tka}, Modern={modern_tka}"
                    )

                # Compare positions
                legacy_pos = legacy_dims["tka_position"]
                modern_pos = modern_dims["tka_position"]
                pos_diff_x = abs(legacy_pos["x"] - modern_pos["x"])
                pos_diff_y = abs(legacy_pos["y"] - modern_pos["y"])

                if pos_diff_x > 10 or pos_diff_y > 10:  # 10px tolerance
                    discrepancies.append(
                        f"TKA position difference: Legacy={legacy_pos}, Modern={modern_pos}"
                    )
