...
class TestType3DetectionIntegration:
    ...
    def test_canonical_type3_letter_detection(self):
        ...
        for letter in type3_letters:
            ...
            assert (
                letter_type_str == "Type3"
            ), f"Letter {letter} should be classified as Type3"
            ...
            ), f"Motion analysis failed for {letter}"
    ...
    def test_type3_dash_positioning_order(self):
        ...
        assert (
            shift_location != dash_location
        ), "Dash should not be placed at shift location"
    ...
    def test_all_type3_motion_combinations(self):
        ...
        for combo in motion_combinations:
            ...
            red_motion = MotionData(
                ...
                prop_rot_dir=(
                    RotationDirection.CLOCKWISE
                    if combo["red"] != MotionType.DASH
                    else RotationDirection.NO_ROTATION
                ),
                start_loc=Location.SOUTH,
                end_loc=(
                    Location.WEST if combo["red"] != MotionType.DASH else Location.NORTH
                ),
            ...
            assert (
                letter_info["letter_type"] == LetterType.TYPE3
            ), f"Type 3 detection failed for {combo['name']}"

            # Test that both arrows can get dash locations calculated
            blue_dash_loc = self.dash_service.calculate_dash_location_from_beat(
                beat_data, is_blue_arrow=True
            )
            red_dash_loc = self.dash_service.calculate_dash_location_from_beat(
                beat_data, is_blue_arrow=False
            )

            assert (
                blue_dash_loc is not None
            ), f"Blue dash location failed for {combo['name']}"
            assert (
                red_dash_loc is not None
            ), f"Red dash location failed for {combo['name']}"
    ...
    def test_type3_specific_scenarios(self):
        ...
        for scenario in scenarios:
            ...
            assert (
                actual_shift == scenario["expected_shift"]
            ), f"Shift location wrong in {scenario['name']}"

            # Verify dash avoids shift
            assert (
                dash_location != actual_shift
            ), f"Dash should avoid shift in {scenario['name']}"
