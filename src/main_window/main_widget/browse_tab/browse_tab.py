...
class BrowseTab(QWidget):
    def __init__(self, main_widget: "MainWidget") -> None:
        ...
        self._apply_saved_browse_state()

    def _apply_saved_browse_state(self):
        left_index = self.settings_manager.get_browse_left_stack_index()
        self.main_widget.left_stack.setCurrentIndex(left_index)

        section_name = self.settings_manager.get_current_section()
        ...
        filter_criteria = self.settings_manager.get_current_filter()
        if filter_criteria:
            self.filter_controller.apply_filter(filter_criteria)

        selected_seq = self.settings_manager.get_selected_sequence()
        if selected_seq:
            word = selected_seq.get("word")
            var_index = selected_seq.get("variation_index", 0)

            from PyQt6.QtCore import QTimer
            QTimer.singleShot(500, lambda: self._reopen_sequence(word, var_index))
    ...
    def _reopen_sequence(self, word: str, var_index: int):
        if word in self.sequence_picker.scroll_widget.thumbnail_boxes:
            ...
            if 0 <= var_index < len(box.state.thumbnails):
                ...
                self.selection_handler.on_box_thumbnail_clicked(box.image_label, metadata)
