...
class ThumbnailBoxHeader(QWidget):
    ...
    def resizeEvent(self, event):
        """Ensures both the difficulty label & favorite button have the exact same width."""
        font_size = self.thumbnail_box.width() // 18
        font = QFont("Georgia", font_size, QFont.Weight.DemiBold)
        self.word_label.setFont(font)

        # Ensure equal width for difficulty label and favorite button
        icon_size = QSize(font_size + 10, font_size + 10)
        max_width = max(self.favorite_button.width(), self.difficulty_label.width())

        # Set both elements to the same width
        self.favorite_button.setIconSize(icon_size)
        self.favorite_button.setFixedSize(max_width, icon_size.height())
        self.difficulty_label.setFixedSize(max_width, icon_size.height())

        # Adjust text positioning
        color = self.settings_manager.global_settings.get_current_font_color()
        self.word_label.setStyleSheet(f"color: {color};")

        available_width = self.thumbnail_box.width() - (max_width * 3)
    ...
    def get_star_outline_icon(
        ...
        if self.settings_manager.global_settings.get_current_font_color() == "black":
            return "black_star_outline.png"
        elif self.settings_manager.global_settings.get_current_font_color() == "white":
