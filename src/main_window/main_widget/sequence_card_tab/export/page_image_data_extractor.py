...
class PageImageDataExtractor:
    ...
    def extract_sequence_data_from_page(self, page: QWidget) -> List[Dict[str, Any]]:
        ...
        if direct_items:
            ...
            self.logger.debug(f"Found {len(direct_items)} sequence items from direct layout")
        
        # If we didn't find any sequence items, try to search recursively through all children
        if not sequence_items:
            self.logger.debug("No sequence items found in direct layout, searching recursively")
            recursive_items = self._extract_sequence_data_recursively(page)
            if recursive_items:
                sequence_items.extend(recursive_items)
                self.logger.debug(f"Found {len(recursive_items)} sequence items from recursive search")
    ...
    def _extract_sequence_data_from_layout(self, page: QWidget) -> List[Dict[str, Any]]:
        ...
        for i in range(layout.count()):
            ...
            if sequence_data:
                ...
                # Add the widget geometry to the sequence data
                item_data = {
                ...
                self.logger.debug(f"Added sequence item {len(sequence_items)}: {word}")
        
        return sequence_items
    
    def _extract_sequence_data_recursively(self, parent_widget: QWidget) -> List[Dict[str, Any]]:
        """
    ...
    def _extract_sequence_data_from_widget(self, widget: QWidget) -> Optional[Dict[str, Any]]:
        """
        ...
        if isinstance(widget, QLabel) and widget.pixmap() and not widget.pixmap().isNull():
            ...
            if widget.property("sequence_data"):
                ...
                if image_path:
                    ...
                    metadata = self.metadata_extractor.extract_metadata_from_file(image_path)
            ...
            if not sequence_data:
                ...
                if image_path and os.path.exists(image_path):
                    ...
                    if metadata:
                        ...
                        word = self._extract_word_from_path_or_metadata(image_path, metadata)
    ...
    def _extract_word_from_path_or_metadata(self, image_path: str, metadata: Dict[str, Any]) -> str:
        """
        ...
        if metadata and "sequence" in metadata:
            ...
            elif isinstance(metadata["sequence"], list) and len(metadata["sequence"]) > 0:
                # The sequence might be a list of dictionaries
                if isinstance(metadata["sequence"][0], dict) and "word" in metadata["sequence"][0]:
