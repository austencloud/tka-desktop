...
class ColorManager:
    ...
    def correct_svg_colors(self, svg_data: str) -> str:
        ...
        for original_color, corrected_color in self.color_correction.items():
            ...
            patterns = [
                ...
                # Class definition
                re.compile(r'(\.(st0|cls-1)\s*\{[^}]*?fill:\s*)(' + re.escape(original_color) + r')([^}]*?\})'),
            ]
            
            # Apply replacements
            for pattern in patterns:
                svg_data = pattern.sub(lambda m: m.group(1) + corrected_color + m.group(len(m.groups())), svg_data)
    ...
    def process_image(self, image: QImage) -> QImage:
        ...
        if self.use_high_bit_depth and image.format() != QImage.Format.Format_ARGB64_Premultiplied:
            # Use 16-bit color depth for better color fidelity
            processed_image = image.convertToFormat(QImage.Format.Format_ARGB64_Premultiplied)
        else:
            # Fallback to 8-bit color depth
            processed_image = image.convertToFormat(QImage.Format.Format_ARGB32_Premultiplied)
        
        # Apply gamma correction if needed
        if self.gamma_correction != 1.0:
            processed_image = self._apply_gamma_correction(processed_image, self.gamma_correction)
        
        # Apply red channel enhancement if needed
        if self.enhance_red_channel != 1.0:
            processed_image = self._enhance_color_channel(processed_image, channel=0, factor=self.enhance_red_channel)
    ...
    def _enhance_color_channel(self, image: QImage, channel: int, factor: float) -> QImage:
        """
        ...
        for y in range(image.height()):
            for x in range(image.width()):
                ...
                # Set the corrected color
                result.setPixelColor(x, y, QColor(channels[0], channels[1], channels[2], color.alpha()))
        ...
        for hex_color, corrected_hex in self.color_correction.items():
            if hex_color.startswith("#") and corrected_hex.startswith("#"):
                try:
                    ...
                    self.logger.warning(f"Invalid color conversion: {hex_color} -> {corrected_hex}: {e}")
        ...
        for y in range(image.height()):
            for x in range(image.width()):
                ...
                for original_rgb, corrected_color in color_map.items():
                    ...
                    if self._color_distance(color, original_color) < 30:  # Adjust tolerance as needed
