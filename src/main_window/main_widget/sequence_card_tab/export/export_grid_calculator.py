...
class ExportGridCalculator:
    ...
    def calculate_optimal_grid_dimensions(
        ...
        if sequence_length is not None:
            ...
            self.logger.debug(f"Using predefined grid dimensions for length {sequence_length}: {rows}x{cols}")
            return rows, cols
        ...
        page_aspect = (
            self.config.get_print_setting("page_width_pixels", 5100)
    ...
    def calculate_image_position_in_cell(
        self, cell_x: int, cell_y: int, cell_width: int, cell_height: int, 
