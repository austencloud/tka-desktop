...
class StrictMirroredPermutationExecutor(PermutationExecutor):
    ...
    def create_permutations(self, sequence: list[dict], vertical_or_horizontal: str):
        """Creates mirrored permutations for a circular sequence."""
        ...
        final_intended_sequence_length = sequence_length + self.determine_how_many_entries_to_add(sequence_length)

        for i in range(2, sequence_length + 2):  # Skip first two beats
            next_pictograph = self.create_new_mirrored_permutation_entry(
                sequence, last_entry, next_beat_number + i - 2, final_intended_sequence_length
            )
            ...
            sequence_workbench.sequence_beat_frame.beat_factory.create_new_beat_and_add_to_sequence(
                next_pictograph, override_grow_sequence=True, update_word=False, update_image_export_preview=False
            )
            QApplication.processEvents()

    def create_new_mirrored_permutation_entry(
        self, sequence, previous_entry, beat_number: int, final_intended_sequence_length: int
    ) -> dict:
        """Generates a new mirrored pictograph entry by flipping attributes."""
        previous_matching_beat = self.get_previous_matching_beat(sequence, beat_number, final_intended_sequence_length)
        ...
        new_entry = {
            ...
            BLUE_ATTRS: self.create_new_attributes(previous_entry[BLUE_ATTRS], previous_matching_beat[BLUE_ATTRS]),
            RED_ATTRS: self.create_new_attributes(previous_entry[RED_ATTRS], previous_matching_beat[RED_ATTRS]),
        }

        # Ensure orientations are set properly
        new_entry[BLUE_ATTRS][END_ORI] = self.circular_sequence_generator.json_manager.ori_calculator.calculate_end_ori(new_entry, BLUE)
        new_entry[RED_ATTRS][END_ORI] = self.circular_sequence_generator.json_manager.ori_calculator.calculate_end_ori(new_entry, RED)

        # Swap colors at the midpoint of the sequence
        if self.color_swap_second_half and beat_number >= final_intended_sequence_length // 2:
    ...
    def get_mirrored_position(self, previous_matching_beat) -> str:
        ...
        return mirrored_positions[self.vertical_or_horizontal].get(
            previous_matching_beat[END_POS], previous_matching_beat[END_POS]  # Fallback to original position
        )


    def create_new_attributes(self, previous_entry_attributes: dict, previous_matching_beat_attributes: dict) -> dict:
        """Creates mirrored attributes by flipping relevant properties."""
        motion_type = previous_matching_beat_attributes[MOTION_TYPE]
        prop_rot_dir = self.get_mirrored_prop_rot_dir(previous_matching_beat_attributes[PROP_ROT_DIR])
        ...
        new_entry_attributes = {
            ...
            END_LOC: self.calculate_mirrored_permutation_new_loc(previous_matching_beat_attributes[END_LOC]),
        ...
        if previous_matching_beat_attributes.get(PREFLOAT_MOTION_TYPE):
            new_entry_attributes[PREFLOAT_MOTION_TYPE] = previous_matching_beat_attributes[PREFLOAT_MOTION_TYPE]
            new_entry_attributes[PREFLOAT_PROP_ROT_DIR] = self.get_mirrored_prop_rot_dir(
    ...
    def calculate_mirrored_permutation_new_loc(self, previous_matching_beat_end_loc: str) -> str:
        """Finds the new mirrored location based on grid mode."""
        if self.vertical_or_horizontal == VERTICAL:
            return vertical_loc_mirror_map.get(previous_matching_beat_end_loc, previous_matching_beat_end_loc)
        elif self.vertical_or_horizontal == HORIZONTAL:
            return horizontal_loc_mirror_map.get(previous_matching_beat_end_loc, previous_matching_beat_end_loc)
